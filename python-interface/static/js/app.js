...
class RobotController {
    ...
    this.mapClickBound = false;
        ...
        this.lastVoiceCommandTs = 0;
        this.lastVoiceCommandText = '';
    ...
    if (clearBreadcrumbsBtn) clearBreadcrumbsBtn.addEventListener('click', () => { this.breadcrumbs = []; this.drawMap(); this.showMessage('Breadcrumbs cleared', 'info'); });
        ...
        if (micFab) micFab.addEventListener('click', () => this.toggleVoice());
    ...
    updateConnectionState(state) {
        ...
        switch(state) {
            case 'connecting':
                ...
                if (micFab) { micFab.style.display = 'none'; micFab.setAttribute('aria-hidden', 'true'); }
                break;
            ...
            case 'connected':
                ...
                if (micFab) { micFab.style.display = 'flex'; micFab.setAttribute('aria-hidden', 'false'); }
                break;
            ...
            default:
                ...
                disconnectBtn.style.display = 'none';
                connectionInfo.style.display = 'none';
                // Hide and stop voice when disconnected
                if (micFab) { micFab.style.display = 'none'; micFab.setAttribute('aria-hidden', 'true'); micFab.setAttribute('aria-pressed', 'false'); }
                if (this.voice && this.voice.recognizing && this.voice.recognizer) {
                    try { this.voice.recognizer.stop(); } catch(_) {}
                }
    ...
    updateDashboardStatus() {
        ...
        const dashPosition = document.getElementById('dashPosition');
            document.addEventListener('keydown', (e) => {
                if (e.key.toLowerCase() === 'm' && !e.repeat && this.connected) {
                    this.toggleVoice();
                }
            });
...
document.addEventListener('DOMContentLoaded', () => {
    window.app = new RobotController();

    // Optional: quick voice test panel without microphone
    if (window.location.search.includes('voicetest=1')) {
        const panel = document.createElement('div');
        panel.style.cssText = 'position:fixed;bottom:12px;right:12px;background:#111827;color:#fff;padding:10px;border-radius:10px;z-index:2000;opacity:0.9;box-shadow:0 6px 18px rgba(0,0,0,0.3);max-width:260px;font-family:Segoe UI, sans-serif;';
        panel.innerHTML = '<div style="font-weight:700;margin-bottom:6px;">Voice Test (dev)</div>';
        const cmds = [
            ['Forward','forward'], ['Backward','backward'], ['Left','left'], ['Right','right'], ['Stop','stop'],
            ['Emergency','emergency stop'], ['Connect','connect'], ['Disconnect','disconnect'],
            ['Faster','faster'], ['Slower','slower'], ['Speed 1.5','speed one point five']
        ];
        const wrap = document.createElement('div');
        wrap.style.display = 'grid';
        wrap.style.gridTemplateColumns = 'repeat(3,1fr)';
        wrap.style.gap = '6px';
        cmds.forEach(([label, phrase]) => {
            const b = document.createElement('button');
            b.textContent = label;
            b.style.cssText = 'background:#374151;color:#fff;border:none;border-radius:6px;padding:6px 8px;cursor:pointer;font-size:12px;';
            b.onclick = () => window.app.handleVoiceCommand(phrase);
            wrap.appendChild(b);
        });
        ...
        close.textContent = '×';
        close.title = 'Close';
        close.style.cssText = 'position:absolute;top:6px;right:8px;cursor:pointer;font-weight:700;';
...
// -----------------------------
RobotController.prototype.initVoice = function() {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) {
        this.voice.supported = false;
        const status = document.getElementById('voiceStatus');
        if (status) status.textContent = 'Not supported in this browser';
        return;
    }
    this.voice.supported = true;
    ...
    recog.onstart = () => {
        ...
        const micFab = document.getElementById('micFab');
        if (micFab) micFab.setAttribute('aria-pressed', 'true');
    };
    ...
    recog.onend = () => {
        ...
        document.body.classList.remove('voice-listening');
        const micFab = document.getElementById('micFab');
        if (micFab) micFab.setAttribute('aria-pressed', 'false');
    };

    recog.onerror = (e) => {
        const status = document.getElementById('voiceStatus');
        if (status) status.textContent = `Error: ${e.error}`;
        this.showMessage(`Voice error: ${e.error}`, 'error');
    };

    recog.onresult = (event) => {
        let interim = '';
        let final = this.voice.final;
        ...
        this.voice.interim = interim;
        this.voice.final = final;
        const transcriptEl = document.getElementById('voiceTranscript');
        if (transcriptEl) {
            transcriptEl.innerHTML = `<span class="final">${final}</span><span class="interim">${interim}</span>`;
        }
    };

    this.voice.recognizer = recog;
};

RobotController.prototype.toggleVoice = function() {
    if (!this.connected) {
        this.showMessage('Connect to the robot to enable voice control.', 'info');
        return;
    }
    if (!this.voice.supported) {
        this.showMessage('Voice not supported in this browser. Use Chrome or Edge desktop.', 'error');
        const micFab = document.getElementById('micFab');
        if (micFab) micFab.classList.add('mic-disabled');
        return;
    }
    if (!this.voice.recognizing) {
        try { this.voice.recognizer.start(); } catch (_) {}
    } else {
        try { this.voice.recognizer.stop(); } catch (_) {}
    }
};

RobotController.prototype.handleVoiceCommand = function(raw) {
    if (!raw) return;
    const phrase = raw.trim().toLowerCase();
    // Global debounce to avoid rapid repeats
    const now = Date.now();
    if (now - this.lastVoiceCommandTs < 350 && phrase === this.lastVoiceCommandText) {
        return;
    }
    this.lastVoiceCommandTs = now;
    this.lastVoiceCommandText = phrase;
    const speak = (msg) => {
    ...
    // Safety: detect emergency phrases first
    if (/\b(emergency stop|panic|stop now|halt)\b/.test(phrase)) {
        this.emergencyStop();
    ...
    // Connection
    if (/\b(connect|start connection|go online)\b/.test(phrase)) {
        this.connectRobot();
        speak('Connecting');
        return;
    }
    if (/\b(disconnect|go offline|stop connection)\b/.test(phrase)) {
    ...
    // Movement
    if (/\b(forward|move forward|go forward)\b/.test(phrase)) {
        this.sendCommand('forward');
        speak('Moving forward');
        return;
    }
    if (/\b(back|backward|reverse|move back)\b/.test(phrase)) {
        this.sendCommand('backward');
        speak('Moving backward');
        return;
    }
    if (/\b(left|turn left)\b/.test(phrase)) {
        this.sendCommand('left');
        speak('Turning left');
        return;
    }
    if (/\b(right|turn right)\b/.test(phrase)) {
        this.sendCommand('right');
        speak('Turning right');
        return;
    }
    if (/\b(stop|pause|freeze|hold)\b/.test(phrase)) {
        this.sendCommand('stop');
        speak('Stopping');
        return;
    }
    ...
    // Speed controls
    if (/\b(faster|speed up|increase speed)\b/.test(phrase)) {
        const slider = document.getElementById('speedSlider');
        if (slider) {
            const v = Math.min(parseFloat(slider.value) + 0.1, parseFloat(slider.max));
            slider.value = v.toFixed(1);
            slider.dispatchEvent(new Event('input'));
            speak(`Speed ${v.toFixed(1)}`);
        }
        return;
    }
    if (/\b(slower|slow down|decrease speed)\b/.test(phrase)) {
        const slider = document.getElementById('speedSlider');
        if (slider) {
            const v = Math.max(parseFloat(slider.value) - 0.1, parseFloat(slider.min));
            slider.value = v.toFixed(1);
            slider.dispatchEvent(new Event('input'));
            speak(`Speed ${v.toFixed(1)}`);
        }
    ...
    // "speed one point five" or "set speed to 1.5"
    const speedMatch = phrase.match(/(speed|set speed(?: to)?|go speed)\s*(to\s*)?([0-9]+(?:\.[0-9]+)?|one(?: point \d+)?|two|three|four|five)/);
    if (speedMatch) {
        let rawNum = speedMatch[3];
        const words = { one: 1, two: 2, three: 3, four: 4, five: 5 };
        if (words[rawNum] !== undefined) rawNum = String(words[rawNum]);
        rawNum = rawNum.replace('point ', '.');
        const num = parseFloat(rawNum);
        if (!isNaN(num)) {
            const slider = document.getElementById('speedSlider');
            if (slider) {
                const clamped = Math.max(parseFloat(slider.min), Math.min(num, parseFloat(slider.max)));
                slider.value = clamped.toFixed(1);
                slider.dispatchEvent(new Event('input'));
                speak(`Speed ${clamped.toFixed(1)}`);
            }
        }
        return;
    }
    ...
    // Waypoint mode
    if (/\b(waypoint mode|toggle waypoint)\b/.test(phrase)) {
        const cb = document.getElementById('toggleWaypointMode');
        if (cb) { cb.checked = !cb.checked; cb.dispatchEvent(new Event('change')); }
        speak('Waypoint mode toggled');
        return;
    }

    // AR toggle
    if (/\b(toggle a\.r\.|toggle ar|toggle a r|toggle view)\b/.test(phrase)) {
        this.toggleAR();
        speak('Toggled AR');
        return;
    }
